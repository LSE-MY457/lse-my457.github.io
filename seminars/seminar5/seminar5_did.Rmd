...
package_check(c("dplyr", "ggplot2", "estimatr", "fixest", "panelView", "did", "fect"))
...
# rdrobust for rdrobust estimators
# rddensity for rdd density estimation
# RDHonest for rdhonest estimators
set.seed(02139)
```
...
data_multi = tibble(
  ...
  C = c(replicate(N, rep(sample(c(0,1),1, prob = c(0.2,0.8)),periods))),
  ...
  mutate(G = ifelse(C == 1, G, NA))

# visualise the treatment regime (staggered and absorbing):
